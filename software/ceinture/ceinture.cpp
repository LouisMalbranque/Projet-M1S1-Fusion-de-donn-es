...
void Ceinture::update(){
  pox.update();
  if (millis() - lastResp > 1000/SAMPLE_FREQUENCY){
    if (i < SAMPLE_NUMBER){
...
double Ceinture::getTemperature(){
  ...
  temperature = sensors.getTempCByIndex(PIN_TEMPERATURE_SENSOR);
  return temperature;
}
...
double Ceinture::getRespiratoryRate(){
  ...
  respiratoryRate = FFT.MajorPeak(vReal, SAMPLE_NUMBER, SAMPLING_FREQUENCY);
